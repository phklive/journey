...
def boar_brawl(player_score, opponent_score):
    ...
    player_digit, opponent_digit = get_digit(player_score, 0), get_digit(opponent_score, 1)
    score = 3 * abs(opponent_digit - player_digit)
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    while score0 < goal and score1 < goal:
        # use the appropriate strategy for the turn player
        player_strategy = strategy0 if who == 0 else strategy1
        num_rolls = player_strategy(score0, score1) if who == 0 else player_strategy(score1, score0)

        # compute the new_score and assign it to the turn player
        if who == 0:
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for i in range(1, 11):
        ...
        if highest_score < new_score:
            highest_score, rolls = new_score, i
    return rolls
    # END PROBLEM 9
